...
Telnet.prototype.processBuffer = function (buffer) {
    ...
    if (result.data.length > 0) {
        if (!_self._authenticated && _self._password.length > 0) _self.login(result.data);
        else _self.emit("response", result.data.toString());
    }
};

Telnet.prototype.login = function (data) {
    var _self = this;
    if (step === 0 && data.toString().match(/(Username[:])\s*$/g) || data.toString().match(/(login[:])\s*$/g)) {
        _self.write(_self._username + "\r\n");
        step++;
    } else if (step === 1 && data.toString().match(/(Password[:])/g)) {
        _self.write(_self._password + "\r\n");
        step++;
    } else if (step === 2 && data.toString().match(/(#)\s*$/) > -1) {
        _self._authenticated = true;
        _self.emit("authenticated");
    } else _self.emit('data', data.toString());
}
